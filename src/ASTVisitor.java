        ...
        try {
            ...
            if (program.getDef_class() != null) {
                for (int i = 0; i < program.getDef_class().size(); i++) {
                    visit(program.getDef_class().get(i), space);
                }
            }
            writeToFile("\n\t}");
            fw.flush();
...
//        }
    ...
    public void visit(Content content, String Space, int parentId, int scope) {
        ...
        if (content.getNavigatorRule() != null) {
            visit(content.getNavigatorRule(), Space, parentId, scope);
        }
...
//        }
    ...
    public void visit(widget widget, String space, int parentId, int scope) {
        ...
        if (widget.getInkWell() != null) {
            visit(widget.getInkWell(), space + '\t', parentId, scope + 1);
        }
    }
    ...
    //textfield ----------------------------------------------------------------------
    public void visit(InkWell inkWell, String space, int parentId, int scope) {
        final int id = ThreadLocalRandom.current().nextInt();
        writeToFile("\n" + space + "InkWell" + ":");
        ...
        if (inkWell.getInkWellBodyList() != null) {
            for (int i = 0; i < inkWell.getInkWellBodyList().size(); i++) {
                visit(inkWell.getInkWellBodyList().get(i), space + '\t', id, scope);
            }
    ...
    public void visit(InkWellBody inkWellBody, String space, int parentId, int scope) {
        final int id = ThreadLocalRandom.current().nextInt();
        if(inkWellBody instanceof InkWellOnPressed){
            ...
            ((InkWellOnPressed) inkWellBody).getContentList().forEach(content -> {
                visit(content,space + "\t",id,scope + 1);
            });
            ...
            visit(((InkWellChild) inkWellBody).getWidget(), space, parentId, scope);
        }
    ...
    public void visit(NavigationRule navigationRule, String space, int parentId, int scope) {
        final int id = ThreadLocalRandom.current().nextInt();
        writeToFile("\n" + space + "Navigator" + ":");
        addToSymbolTable(parentId, new SymbolTableObject(SymbolTableRowType.Navigator, "Navigator", new SymbolTableObjectNavigatorValue(scope, id, parentId, navigationRule.getRouteName())));
    }
